...
class LevelConfiguration(models.Model):
    ...
    name = fields.Char(
        ...
        required=True,
        copy=False,
        help="A descriptive name for this level configuration (e.g., 'Standard Level 1', 'Advanced Level 1 - Therapy Group A')."
    )
    ...
    partner_ids = fields.Many2many(
        ...
        help='Patients this configuration applies to. A patient can only have one active configuration per level number.'
    )
    ...
    _sql_constraints = [
        ('name_level_uniq', 'unique(name, level_number)', 'A configuration with this name and level number already exists!')
    ]
            ...
            for patient in record.partner_ids:
                ...
                if self.search_count(domain) > 0:
                    raise ValidationError(
                        _("Patient %s already has an active configuration for level %s.") %
                        (patient.name, record.level_number)
                    )
    ...
    def _check_time_limit(self):
        for record in self:
            if not (1.0 <= record.coin_challenge_time_limit <= 60.0):
                raise ValidationError(_("Coin challenge time limit must be between 1 and 60 seconds."))

    @api.constrains('level_number', 'forward_pose_id', 'backward_pose_id', 'left_pose_id', 'right_pose_id', 'coin_pose_line_ids') # Updated field name
        ...
        for record in self:
            if record.level_number == 1:
                ...
                if missing_movement_poses:
                    raise ValidationError(
                        _("Level 1 ('%s') requires all movement poses. Missing: %s") %
                        (record.name, ', '.join(missing_movement_poses))
                    )
                # Check the count of coin_pose_line_ids
                if len(record.coin_pose_line_ids) != 3:
                    raise ValidationError(
                        _("Level 1 ('%s') requires exactly 3 coin poses. Currently has: %s") %
                        (record.name, len(record.coin_pose_line_ids))
                    )

    def get_level_data_for_unity(self, partner_id_value, partner_name_value):
        """Returns level configuration data in JSON format for Unity for a specific patient."""
        self.ensure_one()
        ...
        coin_poses_data = {
            f'coin{idx + 1}': get_pose_data(pose_line.pose_id) # Access pose_id from the line record
            for idx, pose_line in enumerate(self.coin_pose_line_ids.sorted('sequence')) # Ensure correct order
        }
        ...
        return {
            ...
            'timing': {
                'coin_challenge_time_limit': self.coin_challenge_time_limit
            },
            'timestamp': fields.Datetime.now().isoformat()
        }
    def get_data_for_unity(self):
        self.ensure_one()
        """Returns level configuration data in JSON format for Unity without patient context without previous function or partner context."""
        def get_pose_data(pose_record):
            if pose_record:
                return pose_record.get_pose_data_for_unity()
            return None
    ...
    def get_level_configuration_for_patient(self, level_number, partner_id):
        ...
        if not level_config:
            raise ValidationError(
                _("No active Level %s configuration found for patient %s (ID: %s), nor a generic configuration for this level.") %
                (level_number, patient.name, partner_id)
            )

        level_data = level_config.get_level_data_for_unity(
            partner_id_value=patient.id,
            partner_name_value=patient.name
        )

        return {
            'success': True,
            'level_data': level_data,
            'message': f"Successfully retrieved Level {level_number} configuration for patient {patient.name}"
        }
    ...
    def get_first_lc_active(self):
        ...
        return {
            'success': True,
            'level_data': level_data,
            'message': f"Successfully retrieved first active level configuration: {level_config.name}"
        }
